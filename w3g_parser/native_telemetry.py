...
HOST_PING = 4
HOST_SHUTDOWN = 5
HOST_MAX_ABILITIES = 32
HOST_HEADER = struct.Struct('<IHHII')
HOST_CONFIG = struct.Struct('<III')
HOST_TARGET = struct.Struct('<II')
HOST_RESPONSE_PREFIX = struct.Struct('<8IQQ')
HOST_ABILITY = struct.Struct('<4I')
HOST_RESPONSE_SIZE = HOST_RESPONSE_PREFIX.size + HOST_MAX_ABILITIES * HOST_ABILITY.size
HOST_STATUS = {1: 'Telemetry Host отклонил IPC-сообщение', 2: 'Telemetry Host ещё не подключён к Warcraft', 3: 'Telemetry Host не смог открыть процесс Warcraft для чтения', 4: 'Game.dll не соответствует проверенному Warcraft III 1.26a (6401)', 5: 'Герой выбранного игрока пока не найден в памяти Warcraft', 6: 'Telemetry Host не смог прочитать живое состояние способностей', 7: 'Warcraft III был закрыт'}

class TelemetryHostError(SeekBackendError):

    def __init__(self, status: int, detail: int, message: str) -> None:
...
def find_native_telemetry_host() -> Path:
    ...
    raise SeekBackendError('Нативный Telemetry Host не найден. Собери read-only C++ модуль командой tools/build_native.py.')
...
class NativeTelemetryHost:
    ...
    def __init__(self, player_slot: int, hero_rawcode: str, *, process_id: int=0, executable: Path | None=None) -> None:
        if os.name != 'nt':
            raise SeekBackendError('Telemetry Host requires Windows')
        ...
        self._closed = False
        try:
            self._exchange(HOST_CONFIGURE, struct.pack('<I', process_id) + target)
        except Exception:
            self.close()
            raise

    @staticmethod
    def _target_payload(player_slot: int, hero_rawcode: str) -> bytes:
        if not 0 <= player_slot <= 15:
            raise ValueError('Player slot must be in range 0..15')
        return HOST_TARGET.pack(player_slot, rawcode_value(hero_rawcode))
    ...
    def _read_exact(stream: object, size: int) -> bytes:
        ...
        while remaining:
            ...
            if not chunk:
                raise SeekBackendError('Telemetry Host terminated unexpectedly')
            chunks.append(chunk)
            remaining -= len(chunk)
    ...
    def _decode_response(payload: bytes) -> tuple[int, int, TelemetrySnapshot]:
        if len(payload) != HOST_RESPONSE_SIZE:
            raise SeekBackendError('Telemetry Host response size mismatch')
        status, detail, process_id, game_dll_base, game_time_ms, hero_address, hero_value, ability_count, memory_read_count, memory_write_count = HOST_RESPONSE_PREFIX.unpack_from(payload)
        if ability_count > HOST_MAX_ABILITIES:
            raise SeekBackendError('Telemetry Host returned too many abilities')
        ...
        return (status, detail, TelemetrySnapshot(process_id=process_id, game_dll_base=game_dll_base, game_time_ms=game_time_ms, hero_address=hero_address, hero_rawcode=hero_rawcode, abilities=tuple(abilities), memory_read_count=memory_read_count, memory_write_count=memory_write_count))

    def _exchange(self, command: int, payload: bytes=b'', *, accept_error: bool=False) -> TelemetrySnapshot:
        with self._lock:
            if self._closed:
                raise SeekBackendError('Telemetry Host is closed')
            stdin = self._process.stdin
            stdout = self._process.stdout
            if stdin is None or stdout is None:
                raise SeekBackendError('Telemetry Host IPC is unavailable')
            self._request_id += 1
            ...
            try:
                ...
                if magic != HOST_MAGIC or version != HOST_PROTOCOL_VERSION or response_command != command or (response_id != request_id) or (response_size != HOST_RESPONSE_SIZE):
                    raise SeekBackendError('Telemetry Host protocol mismatch')
                response = self._read_exact(stdout, response_size)
            except (BrokenPipeError, OSError) as exc:
                raise SeekBackendError(f'Telemetry Host IPC failed: {exc}') from exc
            ...
            if status and (not accept_error):
                ...
                raise TelemetryHostError(status, detail, f'{message}{suffix}')
            return snapshot

    def set_target(self, player_slot: int, hero_rawcode: str) -> TelemetrySnapshot:
        return self._exchange(HOST_SET_TARGET, self._target_payload(player_slot, hero_rawcode))
